...
class FaceRec:
    ...
    def faceRec(self):
        ...
        tempfaceid = self.detectImage(image_data, SUBSCRIPTION_KEY, ENDPOINT)
        if not tempfaceid:
            return_response['status'] = 'Fail'
        else:
            face_id = self.identifyImage(tempfaceid, SUBSCRIPTION_KEY, ENDPOINT)
            if not face_id:
                return_response['status'] = 'Fail'
            else:
    ...
    def detectImage(self, image_data, subscription_key, endpoint):
    ...
    def identifyImage(self, tempfaceid, subscription_key, endpoint):
